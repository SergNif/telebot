        ...
        if len(file_list) > 0:
            for i, mm in enumerate(file_list):
                ...
                if os.path.exists(folder + file_list[i].replace("style_", "")):
                    name.append(folder + file_list[i].replace("style_", ""))     #(folder + file_list[0].split("_")[0] + '.' + file_list[0].split(".")[-1])
                    print(f'{name[1]= }')
                dict_i[i]=name
...
def dict_json_edit(param=None):    
    ...
    if len(dict_image) > 1:
        ...
        if param == 'delete_first':
            index = sorted([i for i in dict_image.keys()])[1] #min(dict_image, key=dict_image.get)
...
def rename_PIC(pic_1, pic_style, user_id, chat_id):
    ...
    rand_text = [random.choice(string.ascii_lowercase + string.digits) if i != 5 else random.choice(string.ascii_uppercase) for i in range(10)]
    ...
    PIC_cont = IMAGE_DIR + str(chat_id) + '__' + str(user_id) + '_' + rand_text + '_.' + pic_1_ext
    PIC_style = IMAGE_DIR + str(chat_id) + '__' + str(user_id) + '_' + rand_text + '_style_.' + pic_style_ext
    PIC_1 = pic_1
    PIC_2 = pic_style
    print(f'{pic_1= }  ')
    print(f'{pic_style= }  ')
    ...
    os.rename(pic_1, PIC_style)     #  Я ГДЕ-ТО ЗАПУТАЛСЯ, ПОЭТОМУ ПОМЕНЯЛ МЕСТАМИ cont и style
...
if __name__ == "__main__":
    uvicorn.run("exam04FEB:app", host="0.0.0.0", workers=3,  port=443, log_level="debug",  reload=True,  ssl_keyfile="/etc/letsencrypt/live/gr.tgram.ml/privkey.pem",
